    ...
    <div class="relative z-40 pl-6">
      {#if user.current}
        <button
          ...
          style="background-color: {user.current.hasImage
            ? 'transparent'
            : getAvatarColor(user.current.name)}"
          onclick={() => (menuOpen = !menuOpen)}
          aria-label="User menu"
        >
          {#if user.current.hasImage}
            <img
              src={createProfilePictureUrl(user.current.id)}
              alt="User avatar"
        ...
        {#if menuOpen}
          <div
            class="absolute right-0 top-full mt-1.5 w-52 overflow-hidden border border-zinc-700 bg-[#0d0d0d] shadow-2xl shadow-black/80"
          >
