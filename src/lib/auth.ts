...
export async function extractSessionId(sessionCookieValue: string): Promise<string | null> {
  try {
...
export async function signOut(sessionToken: string) {
  try {
    const response = await fetch(`${envPublic.PUBLIC_UNO_BASE_URL}/auth/sign-out`, {
      method: 'POST',
      headers: {
...
export async function getUserById(userId: string) {
  const response = await fetch(`${envPublic.PUBLIC_UNO_BASE_URL}/users/${userId}`, {
    headers: {
...
export const getSessionById = async (sessionId: string) => {
  const response = await fetch(`${envPublic.PUBLIC_UNO_BASE_URL}/auth/sessions/${sessionId}`, {
    headers: {
      'X-Admin-Key': envPrivate.ADMIN_KEY,
    },
  })
