<script lang="ts">
	...
	const source = $derived(
		(postStore.getOverride(id) ?? postStore.findById(id) ?? (mouseClicksData as MouseClick[]).find((p) => p.id === id)) as MouseClick | undefined
	);
	...
	{#if source}
		<form onsubmit={submit} class="flex flex-col gap-5">
			<div class="flex flex-col gap-1.5">
				<label for="mc-edit-title" class="text-xs text-zinc-400">title <span class="text-zinc-600">*</span></label>
				<input
					id="mc-edit-title"
					type="text"
					bind:value={title}
			...
			<div class="flex flex-col gap-1.5">
				<label for="mc-edit-url" class="text-xs text-zinc-400">url <span class="text-zinc-600">*</span></label>
				<input
					id="mc-edit-url"
					type="url"
					bind:value={url}
			...
			<div class="flex items-center justify-between border-t border-zinc-800 pt-4">
				<a href="/mouse-clicks/{id}" class="text-sm text-zinc-500 transition-colors hover:text-zinc-200">
					[ cancel ]
				</a>
				<button
					type="submit"
