<script lang="ts">
	...
	const source = $derived(
		(postStore.getOverride(id) ?? postStore.findById(id) ?? (catFoodData as CatFoodPost[]).find((p) => p.id === id)) as CatFoodPost | undefined
	);
	...
	{#if source}
		<form onsubmit={submit} class="flex flex-col gap-5">
			<div class="flex flex-col gap-1.5">
				<label for="cf-edit-title" class="text-xs text-zinc-400">title <span class="text-zinc-600">*</span></label>
				<input
					id="cf-edit-title"
					type="text"
					bind:value={title}
			...
			<div class="flex flex-col gap-1.5">
				<label for="cf-edit-tagline" class="text-xs text-zinc-400">tagline <span class="text-zinc-600">*</span></label>
				<input
					id="cf-edit-tagline"
					type="text"
					bind:value={tagline}
			...
			<div class="flex items-center justify-between border-t border-zinc-800 pt-4">
				<a href="/cat-food/{id}" class="text-sm text-zinc-500 transition-colors hover:text-zinc-200">
					[ cancel ]
				</a>
				<button
					type="submit"
