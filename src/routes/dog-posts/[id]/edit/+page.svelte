<script lang="ts">
	...
	const source = $derived(
		(postStore.getOverride(id) ?? postStore.findById(id) ?? (dogPostsData as DogPost[]).find((p) => p.id === id)) as DogPost | undefined
	);
	...
	const valid = $derived(title.trim().length > 0 && tagline.trim().length > 0 && body.trim().length > 0);
	...
	{#if source}
		<form onsubmit={submit} class="flex flex-col gap-5">
			<div class="flex flex-col gap-1.5">
				<label for="dp-edit-title" class="text-xs text-zinc-400">title <span class="text-zinc-600">*</span></label>
				<input
					id="dp-edit-title"
					type="text"
					bind:value={title}
			...
			<div class="flex flex-col gap-1.5">
				<label for="dp-edit-tagline" class="text-xs text-zinc-400">tagline <span class="text-zinc-600">*</span></label>
				<input
					id="dp-edit-tagline"
					type="text"
					bind:value={tagline}
			...
			<label class="flex cursor-pointer items-center gap-3 self-start">
				<div class="relative flex items-center">
					...
					<div class="h-4 w-4 border border-zinc-700 bg-zinc-900 transition-colors peer-checked:border-red-500/50 peer-checked:bg-red-500/10"></div>
					<div class="pointer-events-none absolute inset-0 hidden items-center justify-center peer-checked:flex">
						<span class="text-[10px] text-red-400 leading-none">✓</span>
					</div>
				</div>
				<span class="text-sm text-zinc-400">mark as <span class="text-red-400">[!] important</span></span>
			</label>

			<div class="flex items-center justify-between border-t border-zinc-800 pt-4">
				<a href="/dog-posts/{id}" class="text-sm text-zinc-500 transition-colors hover:text-zinc-200">
					[ cancel ]
				</a>
				<button
					type="submit"
